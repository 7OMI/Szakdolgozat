...
$(function () {
  ...
  $('select#textFilterType').on('change', function () {
    ...
    var smallPlaceholder = $(this).parent().find('u');
    var text = $(this).children(":selected").text().substring(3);
...
function copyTable() {
  ...
  $('table#clist > tbody > tr').each(function () {
    ...
    if (!tr.hasClass('rowSpc')) {
      if (tr.hasClass('rowTop')) {
        text[i] = [];
      }
      $(this).children('td').each(function () {
        var td = $(this);
        if (tr.hasClass('rowTop') && td.hasClass('qty')) {
          // db
          text[i][2] = td.text() + "\t";
        } else if (tr.hasClass('rowTop') && td.hasClass('prN')) {
          // nettó
          text[i][4] = td.text().replace(/([0-9 \u00A0]{1,})Ft/g, '$1').replace(/[ \u00A0]/g, '') + "\n";
        } else if (tr.hasClass('rowTop') && td.attr('title') !== undefined) {
          // név
          text[i][1] = td.attr('title') + "\t";
        } else if (tr.hasClass('rowBtm') && td.hasClass('prG')) {
          // bruttó
          text[i][3] = td.text().replace(/\(br\. ([0-9 \u00A0]{1,}) Ft\)/g, '$1').replace(/[ \u00A0]/g, '') + "\t";
        } else if (tr.hasClass('rowBtm')) {
          // kieginfó
...
function pagination_(current_page, maxelem, limit) {
  // Bejövő adatok
  var page = current_page != null ? Math.abs(current_page) : 1; // Oldalszám

  // Beállítások
  var linklimitZ = 3; // Ennyi link (oldalszám) lesz kint egyszerre

  // Kiszámolósdi magától
  var maxpage = Math.ceil(maxelem / limit); // Oldalak száma (maximum)
  //$maxpage = ;
  ...
  //lapozó linkek
  var linklimit = page < Math.ceil(linklimitZ / 2) + 2 || page > maxpage - (Math.ceil(linklimitZ / 2) + 1) ? linklimitZ + 2 : linklimitZ;
  // $linklimit = ( $page < 6 || $page > $maxpage-5 ? $linklimitZ + 2 : $linklimitZ);

  // ha maxpage annyi mint a limit vagy több (de max 3-mmal, utána ok)
  var kell_vagy_nem = maxpage >= linklimitZ && maxpage <= linklimitZ + 2 ? false : true; // nem-kell // kell

  var linklimit2 = Math.floor(linklimit / 2); // ennyi link legyen az aktuális előtt és után ( vagy amennyi van )
  var linkoffset = page > linklimit2 ? page - linklimit2 - 1 : 0; // átugrott linkek száma
  var linkend = linkoffset + linklimit; // az utolsó oldalszám.
  ...
  for (var i = 1 + linkoffset; i <= linkend && i <= maxpage; i++) {
...
function createPageButton(page) {
  ...
  var label = type != null ? labels[type] : page;
  var button = null;
  if (page != null) {
    ...
    if (page != '#') {
      button.addEventListener("click", function () {
        var postData = $("#prod_filter").serializeArray();
        postData.push({
          name: 'page',
...
function refreshTable(products, pagination) {
  ...
  $.each(products, function (index, product) {
    sorsz++;
    var rowPar = sorsz % 2 > 0 ? 'rowPar' : ''; // páros sor
    var hlProd = ''; // $_GET['hl'] == product.id ? ' hl' : ''; // kiemelt termék

    if ($.isNumeric(product.barcode)) {
      if (product.barcode.length == 13) {
        product.barcode = product.barcode.replace(/([\d]{1})([\d]{6})([\d]{6})/gmi, '$1&nbsp;$2&nbsp;$3');
      } // EAN-13
      else if (product.barcode.length == 12) {
        product.barcode = product.barcode.replace(/([\d]{1})([\d]{5})([\d]{5})([\d]{1})/gmi, '$1&nbsp;$2&nbsp;$3&nbsp;$4');
      } // UPC
    }
    ...
    $.each(product.distributors, function (i, dis) {
      disList.name.push(dis.name);
      disList.code.push(dis.code);
    });
    var priceNet = product.price.net.min > 0 ? product.price.net.min : product.price.net.max;
    var priceGross = product.price.gross.min > 0 ? product.price.gross.min : product.price.gross.max;
    $('#prodlist').append("\n        <tr id=\"CID_".concat(product.id, "\" class=\"rowTop ").concat(rowPar, " ").concat(hlProd, "\">\n          <td title=\"").concat(product.name, "\">").concat(product.name, "</td>\n          <td class=\"qty\">").concat(product.quantity, "</td>\n          <td class=\"prN\">").concat(priceNet != null ? priceNet.toLocaleString('hu-HU') + ' Ft' : '─', "</td>\n          <td rowspan=\"2\" style=\"font-size:0;\"><a class=\"button s\" href=\"/product/").concat(product.id, "/edit\" title=\"Szerkeszt\xE9s\"><i class=\"fa fa-pencil\"></i></a></td>\n        </tr>\n        <tr class=\"rowBtm ").concat(rowPar, " ").concat(hlProd, "\">\n          <td colspan=\"2\">\n            <span class=\"subinfo\">\n              ").concat(disList.name.length > 0 ? "<span title=\"Forgalmaz\xF3: ".concat(disList.name.join(', '), "\"><i class=\"fa fa-sitemap\"></i>").concat(disList.code.join(', '), "</span>") : '', "\n              ").concat(product.barcode.length > 0 ? " <span title=\"Vonalk\xF3d\"><i class=\"fa fa-barcode\"></i><code>".concat(product.barcode, "</code></span>") : '', "\n              ").concat(product.brand != null ? " <span title=\"M\xE1rka: ".concat(product.brand.name, "\"><i class=\"fa fa-yelp\"></i>").concat(product.brand.code, "</span>") : '', "\n              ").concat(product.sku != null ? " <span title=\"Cikksz\xE1m\"><i class=\"fa fa-hashtag\"></i>".concat(product.sku, "</span>") : '', "\n            </span>\n          </td>\n          <td class=\"prG\"><small>").concat(priceGross != null ? '(br. ' + priceGross.toLocaleString('hu-HU') + ' Ft)' : '&nbsp;', "</small></td>\n        </tr>\n        <tr class=\"rowSpc\"></tr>\n      "));
  });

  // oldalszámok
  //$('#pagination').html(pagination_(pagination.current, pagination.max, pagination.limit));
  $('#pagination').append(pagination_(pagination.current, pagination.max, pagination.limit)).append("<span class=\"maxRes\" title=\"".concat(pagination.max, " tal\xE1lat\"><i class=\"fa fa-filter\"></i>").concat(pagination.max, "</span>"));
